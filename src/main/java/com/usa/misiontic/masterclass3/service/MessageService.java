...
public class MessageService {
    ...
    public Optional<Message> getLibrary(int id){
        return messageRepository.getReservation(id);
    }
    public Message save(Message p){
        if(p.getIdMessage()==null){
            return messageRepository.save(p);
        }else{
            Optional<Message> e = messageRepository.getReservation(p.getIdMessage());
            if(e.isPresent()){
                return p;
            }else{
                return messageRepository.save(p);
    ...
    public Message update(Message p){
        if(p.getIdMessage()!=null){
            Optional<Message> q = messageRepository.getReservation(p.getIdMessage());
            if(q.isPresent()){
    ...
    public boolean delete(int id){
        boolean flag=false;
        Optional<Message>p= messageRepository.getReservation(id);
        if(p.isPresent()){
